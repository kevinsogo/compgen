...
**Polygon note:** Due to the way Polygon works, we have to make some hacks so that we are able to use this there. In particular, we will require that *any `import`, aside from builtin packages, must be of the following form*: `from xxx import *`. (It should always be `*`; only `xxx` will be replaced.) Also, you cannot upload anything here directly into Polygon; you have to run the following command first:

```
polygonate formatter.py validator.py 
```

This will generate a folder called `polygon_ready`; the files inside can now be uploaded. 
